    ...
    try:
        ...
        with database.get_session() as session:
            ...
            if fetch_results and fetch_results.get("papers_stored", 0) > 0:
                ...
                # FIX: order by updated_at, not created_at.
                # upsert() only bumps updated_at on existing rows — created_at
                # stays at the original insert time forever. Ordering by
                # created_at can return stale rows from a previous run that
                # share the same created_at timestamp as freshly-parsed rows.
                papers = session.query(Paper).order_by(desc(Paper.updated_at)).limit(fetch_results["papers_stored"]).all()
            else:
                cutoff_date = datetime.now(timezone.utc) - timedelta(days=1)
                # FIX: filter by updated_at for the same reason as above.
                papers = session.query(Paper).filter(Paper.updated_at >= cutoff_date).all()
